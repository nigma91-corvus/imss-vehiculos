...
COLUMNAS_OFICIALES = [
    "No. Ecco.", "Tipo", "Linea", "UBICACIÓN", "Arrendadora", 
    "Estatus", "Placas", "VIN", "No_TC", "Ultimo_Servicio", 
    "CUOTA DIARIA", "TOTAL DÍAS DE SERVICIO", "COSTO MENSUAL SIN IVA (a)", 
    "TOTAL DE DEDUCCIÓN", "TOTAL A PAGAR (b)"
]
...
RUTA_VEHICULOS_LOCAL = os.path.join(BASE_DIR, "img", "vehiculos")

def obtener_ruta_vehiculos_existente():
    if os.path.exists(RUTA_VEHICULOS_LOCAL):
        return RUTA_VEHICULOS_LOCAL
    for letra in ["E", "D", "F", "C", "G", "H"]:
        ruta_alt = f"{letra}:\\img\\vehiculos"
        if os.path.exists(ruta_alt):
            return ruta_alt
    return RUTA_VEHICULOS_LOCAL

RUTA_VEHICULOS_FINAL = obtener_ruta_vehiculos_existente()
...
def obtener_imagen_catalogo(tipo_o_linea):
    catalogo_archivos = {
        "creta": "creta-1-5l-gls-ivt.png",
        "suv": "creta-1-5l-gls-ivt.png",
        "f-150": "f-150-xl.png",
        "pick up": "f-150-xl.png",
        "urvan": "urvan-panel.png",
        "van": "urvan-panel.png",
        "van pasajeros": "urvan-panel.png",
        "chasis cabina": "urvan-panel.png",
        "v-drive": "v-drive-tm-ac.png",
        "sedán": "v-drive-tm-ac.png",
        "sedan": "v-drive-tm-ac.png"
    }
    
    key = str(tipo_o_linea).lower().strip()
    archivo_encontrado = None
    for k, v in catalogo_archivos.items():
        if k in key:
            archivo_encontrado = v
            break
            
    if not archivo_encontrado:
        archivo_encontrado = "v-drive-tm-ac.png"
        
    ruta_dinamica = os.path.join(RUTA_VEHICULOS_FINAL, archivo_encontrado)
    ruta_assets = os.path.join(BASE_DIR, "assets", archivo_encontrado)
    
    if os.path.exists(ruta_dinamica):
        return ruta_dinamica
    elif os.path.exists(ruta_assets):
        return ruta_assets
    return None
...
os.makedirs(os.path.join("img", "vehiculos"), exist_ok=True)
...
with st.sidebar:
    if logo_base64:
        st.markdown(f'<div style="text-align: center; margin-bottom: 15px;"><img src="{logo_base64}" style="width: 110px; height: auto;"></div>', unsafe_allow_html=True)
    else:
        st.markdown("<h2 style='color:#D3C281; text-align:center;'>IMSS</h2>", unsafe_allow_html=True)
...
with col_logo:
    if logo_base64:
        st.markdown(f'<img src="{logo_base64}" style="width: 65px; height: auto; margin-top: 5px;">', unsafe_allow_html=True)

with col_titulo:
    st.markdown('<p class="titulo-principal">Sistema de Gestión y Control Vehicular</p>', unsafe_allow_html=True)
    st.caption(f"**Flotilla Seleccionada:** `{st.session_state.categoria_seleccionada}` | **Módulo Activo:** `{st.session_state.modulo_activo}` | **Fecha de Operación:** {datetime.now().strftime('%d/%m/%Y')}")
...
if mod_actual == "Dashboard General":
    ...
    ecos_en_taller = {r['ECO'] for r in st.session_state.taller_registros if r['Estatus'] == 'Activo (En Taller)' and r['ECO'] in ecos_filtrados}
    ...
    st.markdown("---")
    col_dona, col_barras, col_tabla = st.columns([1.2, 1.5, 1.3], gap="medium")

    with col_dona:
        st.markdown("##### **Estatus Operativo**")
        valores_dona = [n_activos, n_sust, n_taller, n_baja]
        etiquetas_dona = ['Activas', 'Sustitutos', 'En Taller', 'Baja / Inoperativos']
        colores_dona = ['#13382C', '#D3C281', '#8A1538', '#7f8c8d']

        fig_d, ax_d = plt.subplots(figsize=(3.5, 3.5))
        if tot_unidades == 0 and n_taller == 0:
            ax_d.text(0.5, 0.5, 'Sin Datos', horizontalalignment='center', verticalalignment='center', fontsize=12, color='gray')
            ax_d.axis('off')
        else:
            wedges, _ = ax_d.pie(valores_dona, startangle=140, colors=colores_dona, wedgeprops=dict(width=0.4, edgecolor='white', linewidth=2))
            ax_d.legend(wedges, [f"{e}: {v}" for e, v in zip(etiquetas_dona, valores_dona)], loc="center", bbox_to_anchor=(0.5, -0.15), frameon=False, fontsize=8)
            ax_d.axis('equal')
        fig_d.tight_layout()
        st.pyplot(fig_d)
    ...
    with col_barras:
        ...
        if not df_dash.empty and 'Tipo' in df_dash.columns:
            resumen_tipo = df_dash.groupby('Tipo').size().reset_index(name='Cantidad').sort_values(by='Cantidad', ascending=False)
            bars = ax_v.bar(resumen_tipo['Tipo'], resumen_tipo['Cantidad'], color='#13382C')
            ax_v.tick_params(axis='x', rotation=30, labelsize=8)
            ax_v.grid(axis='y', linestyle='--', alpha=0.5)
            for bar in bars:
                h = bar.get_height()
                ax_v.text(bar.get_x() + bar.get_width()/2, h + 0.5, f'{int(h)}', ha='center', va='bottom', fontweight='bold', fontsize=8)
        else:
            resumen_tipo = pd.DataFrame(columns=['Tipo', 'Cantidad'])
            ax_v.text(0.5, 0.5, 'Sin Datos', horizontalalignment='center', verticalalignment='center', fontsize=12, color='gray')
            ax_v.axis('off')
        fig_v.tight_layout()
        st.pyplot(fig_v)
...
# -----------------------------------------------------------------------------
    ...
    lista_ciudades = list(df_base['UBICACIÓN'].dropna().unique()) if 'UBICACIÓN' in df_base.columns else []
    ciudad_sel = st.selectbox("Seleccionar Vista / Filtro de Ciudad:", ["Todas las Ciudades (General)"] + lista_ciudades)
    ...
    if not df_base.empty and 'UBICACIÓN' in df_base.columns:
        ...
        df_ciudades['Movilidad (%)'] = np.where(
            ...
            ((df_ciudades['Titulares_Activos'] + df_ciudades['Sustitutos_Entregados']) / df_ciudades['Flotilla_Asignada'] * 100).round(1),
            0.0
        )
        df_ciudades['Estado'] = np.where(df_ciudades['Movilidad (%)'] >= 95, "VERDE", np.where(df_ciudades['Movilidad (%)'] >= 85, "AMARILLO", "ROJO"))
        df_ciudades.rename(columns={"UBICACIÓN": "Ciudad / OOAD", "Flotilla_Asignada": "Flotilla Asignada", "Titulares_Activos": "Titulares Activos", "Sustitutos_Entregados": "Sustitutos Entregados", "En_Taller_Inoperativos": "En Taller / Inoperativos"}, inplace=True)
    else:
        df_ciudades = pd.DataFrame(columns=["Ciudad / OOAD", "Flotilla Asignada", "Titulares Activos", "Sustitutos Entregados", "En Taller / Inoperativos", "Movilidad (%)", "Estado"])
    ...
    if df_ciudades.empty:
        ...
        if ciudad_sel == "Todas las Ciudades (General)":
            ...
            colores_v = ['#27ae60' if m >= 95 else ('#f39c12' if m >= 85 else '#c0392b') for m in df_ciudades["Movilidad (%)"]]
            bars = ax_v.bar(df_ciudades["Ciudad / OOAD"], df_ciudades["Movilidad (%)"], color=colores_v)
            ax_v.axhline(95, color='#27ae60', linestyle='--', linewidth=1, label="Meta (95%)")
            ax_v.set_ylim(0, 105)
            ax_v.legend(loc="lower right", fontsize=8)
            for bar in bars:
                h = bar.get_height()
                ax_v.text(bar.get_x() + bar.get_width()/2, h + 1, f'{h}%', ha='center', va='bottom', fontweight='bold', fontsize=8)
            fig_v.tight_layout()
            st.pyplot(fig_v)
        else:
        ...
        def colorear_estado(val):
            if val == "VERDE": return 'background-color: #27ae60; color: white; font-weight: bold;'
            elif val == "AMARILLO": return 'background-color: #f39c12; color: white; font-weight: bold;'
...
# -----------------------------------------------------------------------------
    ...
    if cat_actual == "Institucionales":
        ...
        if disponibles >= 0:
            ...
            k3.metric("Saturación de Pool", f"{abs(disponibles)} Excedidas", delta_color="inverse")
            st.error(f"⚠️ ALERTA DE CAPACIDAD: Se han asignado {asignadas} unidades sustitutas, superando el límite del pool contractual ({existentes}).")

        st.markdown("---")
        st.markdown("##### **Solicitudes y Entregas de Sustitutos en Seguimiento (SLA 48 horas)**")
        st.table(pd.DataFrame(columns=["ECO Titular", "Ciudad / OOAD", "Fecha/Hora Ingreso Taller", "ECO Sustituto Asignado", "Estatus Cumplimiento SLA"]))
...
# -----------------------------------------------------------------------------
    ...
    st.info(f"🔒 Módulo restringido para la carga exclusiva de la flotilla actual: **{cat_actual}**.")
    ...
    if st.session_state.admin_autenticado:
        st.markdown(f"##### **Subir Archivo de Plantilla para: {cat_actual} (.xlsx o .csv)**")
        up_file = st.file_uploader("Cargar libro de Excel o CSV con la estructura oficial:", type=["xlsx", "csv"])
        if up_file is not None:
            if st.button("Procesar y Reemplazar Base Activa"):
                try:
                    cat_limpia = cat_actual.lower().replace(" ", "_").replace("/", "").replace("ó", "o")
                    if up_file.name.endswith('.csv'):
                        df_subido = pd.read_csv(up_file, dtype=str)
                        df_subido.to_csv(os.path.join("data", f"plantilla_{cat_limpia}.csv"), index=False)
                    else:
                        df_subido = pd.read_excel(up_file, dtype=str)
                        df_subido.to_excel(os.path.join("data", f"plantilla_{cat_limpia}.xlsx"), index=False)
                    ...
                    st.cache_data.clear()
                    st.success(f"¡Base de datos para {cat_actual} cargada con éxito! Se registraron {len(df_subido)} unidades.")
                    st.rerun()
                except Exception as e:
...
# -----------------------------------------------------------------------------
    ...
    st.markdown(f'<p class="subtitulo-seccion">Expediente Técnico y Documental por ECO - {cat_actual}</p>', unsafe_allow_html=True)
    
    lista_ecos = list(df_base['No. Ecco.'].unique()) if not df_base.empty and 'No. Ecco.' in df_base.columns else []
    
    if not lista_ecos:
        st.warning(f"No hay vehículos cargados en la base de datos para la flotilla **{cat_actual}**.")
    else:
        eco_search = st.selectbox("Seleccione o Ingrese el ECO a Consultar:", lista_ecos, index=0)
        vehiculo_sel = df_base[df_base['No. Ecco.'] == eco_search]
        
        if not vehiculo_sel.empty:
            v_data = vehiculo_sel.iloc[0]
            st.markdown("---")
            st.markdown(f"#### 📋 Ficha Técnica y Descriptiva — ECO: `{v_data['No. Ecco.']}`")
            ...
            with col_info_cat:
                en_taller = any(r['ECO'] == v_data['No. Ecco.'] and r['Estatus'] == 'Activo (En Taller)' for r in st.session_state.taller_registros)
                estatus_veh = "En Taller" if en_taller else str(v_data.get('Estatus', 'Titular Activo'))
                badge_class = "badge-verde" if "Activo" in estatus_veh else ("badge-amarillo" if "Taller" in estatus_veh else "badge-rojo")
            ...
            st.markdown("---")
            t1, t2, t3 = st.tabs(["Galería de Inspección Física", "Expediente Documental (PDF/Visor)", "Historial de Mantenimientos"])
...
# -----------------------------------------------------------------------------
    ...
    tab_captura, tab_editar = st.tabs(["📝 Captura de Altas / Salidas", "✏️ Editar / Corregir Registro Existente"])
    ...
    with tab_captura:
        ...
        if opcion_taller == "1. Ingreso a Taller (Mantenimiento Preventivo / Correctivo)":
            with st.form(key="form_ingreso_mantenimiento"):
                ...
                c1, c2 = st.columns(2)
                eco_t = c1.selectbox("Número Económico (ECO):", lista_ecos_taller if lista_ecos_taller else ["Sin ECCOS registrados"])
                tipo_mantenimiento = c2.selectbox("Tipo de Estatus / Servicio:", ["Mantenimiento Preventivo", "Mantenimiento Correctivo"])
                ...
                req_sust = "Sí" if tipo_mantenimiento == "Mantenimiento Correctivo" else "No"
                if tipo_mantenimiento == "Mantenimiento Correctivo":
                    st.info("ℹ️ **Mantenimiento Correctivo:** Requiere asignación de Vehículo Sustituto (Pool 20%).")
                else:
                    st.caption("ℹ️ **Mantenimiento Preventivo:** No aplica vehículo sustituto si la salida del taller no pasa de 48Hrs.")

                evidencia = st.file_uploader("Subir Diagnóstico / Orden de Entrada (PDF/JPG):", type=["pdf", "jpg", "png"])
                ...
                if st.form_submit_button("Registrar Ingreso a Taller"):
                    if not lista_ecos_taller:
                        ...
                        nombre_archivo = f"{eco_t}_ENTRADA_TALLER_{datetime.now().strftime('%Y%m%d')}.pdf" if evidencia else "N/A"
                        nuevo_reg = {
                            "ECO": eco_t,
            ...
            with st.form(key="form_ingreso_siniestro"):
                ...
                s1, s2 = st.columns(2)
                eco_s = s1.selectbox("Número Económico (ECO):", lista_ecos_taller if lista_ecos_taller else ["Sin ECOs cargados"])
                aseg = s2.selectbox("Aseguradora:", ["Qualitas", "GNP", "AXA", "Banorte", "Inbursa", "Otra"])
                ...
                evidencia_s = st.file_uploader("Declaración de Siniestro / Fotos Impacto (PDF/JPG):", type=["pdf", "jpg", "png"])
                ...
                if st.form_submit_button("Registrar Siniestro e Ingreso"):
                    if not lista_ecos_taller:
                        ...
                        nombre_archivo_s = f"{eco_s}_SINIESTRO_{datetime.now().strftime('%Y%m%d')}.pdf" if evidencia_s else "N/A"
                        nuevo_reg_s = {
                            "ECO": eco_s,
                            "Tipo": "Siniestro",
            ...
            eco_salida = st.selectbox("Ingresar ECO de la Unidad que Saldrá del Taller:", lista_ecos_taller if lista_ecos_taller else ["Sin ECOs cargados"])
            ingresos_activos = [r for r in st.session_state.taller_registros if r["ECO"] == eco_salida and r["Estatus"] == "Activo (En Taller)"]
            
            if len(ingresos_activos) == 0:
                st.error("⚠️ Vehículo sin registro de entrada activo en taller.")
            else:
                reg_previo = ingresos_activos[0]
                st.success(f"✓ Entrada activa confirmada para **{eco_salida}** ({reg_previo['Tipo']} | Fecha Entrada: {reg_previo['Fecha_Ingreso']}).")
                ...
                with st.form(key="form_salida_taller"):
                    ...
                    recibe = st.text_input("Nombre del Personal que Recibe la Unidad:", value="")
                    evidencia_salida = st.file_uploader("Comprobante de Entrega / Conformidad (PDF/JPG):", type=["pdf", "jpg", "png"])
                    ...
                    if st.form_submit_button("Confirmar y Liberar Salida"):
                        nombre_archivo_sal = f"{eco_salida}_SALIDA_TALLER_{datetime.now().strftime('%Y%m%d')}.pdf" if evidencia_salida else "N/A"
    ...
    with tab_editar:
        ...
        if len(st.session_state.taller_registros) == 0:
            ...
            opciones_reg = [
                f"ID: {idx} | ECO: {r['ECO']} | Tipo: {r['Tipo']} | Fecha: {r['Fecha_Ingreso']} | Estatus: {r['Estatus']}"
                for idx, r in enumerate(st.session_state.taller_registros)
            ]
            sel_str = st.selectbox("Seleccione el registro que desea modificar o corregir:", opciones_reg)
            ...
            with st.form(key="form_corregir_taller_extension"):
                ...
                tipos_m_list = ["Mantenimiento Preventivo", "Mantenimiento Correctivo", "Siniestro"]
                idx_t = tipos_m_list.index(reg_actual['Tipo']) if reg_actual['Tipo'] in tipos_m_list else 0
                e_tipo = ce2.selectbox("Tipo Correcto:", tipos_m_list, index=idx_t)
                
                ce3, ce4 = st.columns(2)
                e_resp = ce3.text_input("Responsable:", value=reg_actual['Responsable'])
                e_taller = ce4.text_input("Taller:", value=reg_actual['Taller'])
                
                estatus_list = ["Activo (En Taller)", "Concluido (Salida Completa)", "Anulado por Error"]
                idx_est = estatus_list.index(reg_actual['Estatus']) if reg_actual['Estatus'] in estatus_list else 0
                e_estatus = st.selectbox("Estatus del Registro:", estatus_list, index=idx_est)
                
                e_obs = st.text_area("Observaciones o notas de la corrección:", value=reg_actual['Observaciones'])
                ...
                if st.form_submit_button("💾 Guardar Cambios en Bitácora"):
                    st.session_state.taller_registros[idx_sel] = {
                        ...
                        "Responsable": e_resp,
                        "Taller": e_taller,
                        "Sustituto": "Sí" if e_tipo != "Mantenimiento Preventivo" else "No",
                        "Estatus": e_estatus,
                        "Observaciones": e_obs
                    }
...
# -----------------------------------------------------------------------------
    ...
    st.markdown(f'<p class="subtitulo-seccion">Reasignación Geográfica de Vehículos por Necesidad de Servicio</p>', unsafe_allow_html=True)
    st.info("Permite la transferencia oficial de unidades entre sedes u OOAD por necesidades operativas o de cobertura.")

    lista_ecos_reasignacion = list(df_base['No. Ecco.'].unique()) if not df_base.empty and 'No. Ecco.' in df_base.columns else []

    with st.form(key="form_reasignacion"):
        st.markdown("##### **Formulario Oficial de Reasignación**")
        col_r1, col_r2 = st.columns(2)
        eco_r = col_r1.selectbox("Seleccione el ECO a Reasignar:", lista_ecos_reasignacion if lista_ecos_reasignacion else ["Sin ECOs cargados"])
        ...
        sedes_opt = ["Aguascalientes", "Colima", "Manzanillo", "Tepic", "Mazatlán", "Zacatecas"]
        ...
        if st.form_submit_button("Registrar y Transferir Unidad"):
            if not lista_ecos_reasignacion:
                ...
                st.success(f"La unidad {eco_r} ha sido reasignada exitosamente de {sede_origen} a {sede_destino}.")
...
# -----------------------------------------------------------------------------
    ...
    st.markdown(f'<p class="subtitulo-seccion">Módulo Consolidado de Reportes y Exportación - {cat_actual}</p>', unsafe_allow_html=True)
    st.markdown("##### **Parámetros de Reporte**")
    rc1, rc2, rc3 = st.columns(3)
    tipo_rep = rc1.selectbox("Tipo de Reporte:", ["Inventario General de Flotilla", "Estatus de Movilidad por Ciudad", "Bitácora Mantenimiento y Siniestros", "Consolidado Deducciones y Pagos"])
    formato_rep = rc2.selectbox("Formato de Salida:", ["Excel (.xlsx)", "CSV (.csv)"])
    periodo_rep = rc3.selectbox("Periodo:", ["Septiembre 2026", "Agosto 2026", "Histórico Acumulado"])

    st.markdown("---")
    if st.button("🚀 Generar y Descargar Reporte Consolidado"):
        st.success(f"Reporte '{tipo_rep}' generado en formato {formato_rep}. Descarga iniciada automáticamente.")

# -----------------------------------------------------------------------------
# 9. CONCILIACIÓN FINANCIERA Y PAGOS
# -----------------------------------------------------------------------------
elif mod_actual == "Conciliación Financiera y Pagos":
    st.markdown(f'<p class="subtitulo-seccion">Conciliación Financiera y Control de Pagos Mensuales - Flotilla {cat_actual}</p>', unsafe_allow_html=True)
    st.info("💡 **Acumulación de Archivos:** Las capturas mensuales se integran históricamente para auditar la vigencia completa del contrato.")
    
    archivo_p = st.file_uploader("Cargar Archivo Mensual de Conciliación (.xlsx):", type=["xlsx"])
    df_p = pd.read_excel(archivo_p) if archivo_p is not None else df_base.copy()

    f1, f2 = st.columns(2)
    mes_corte = f1.selectbox("Mes de Corte a Consultar:", ["Acumulado Histórico Total", "Septiembre 2026", "Agosto 2026", "Julio 2026"])
    arr_opciones = ["Todas"] + (list(df_p['Arrendadora'].dropna().unique()) if 'Arrendadora' in df_p.columns else [])
    ...
    c_m2.metric("Deducciones Totales Aplicadas", f"${monto_ded:,.2f}", delta_color="inverse")
